...
// surface iframes the bundle at /apps/run/model-benchmarking). No path prefix.
export default defineConfig({
  ...
  test: {
    ...
    //  - `node`: pure-logic unit tests (*.test.ts) — no DOM (ecosystem matcher,
    //            top-N-by-votes, dedup, data-model parse/migrate, WorkflowBody).
    //  - `dom` : component + e2e tests (*.test.tsx) — jsdom + testing-library,
    //            driving the App against the SDK mock host.
    projects: [
      {
        extends: true,
        test: {
